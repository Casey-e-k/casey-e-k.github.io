...
Several design aspects are still in progress and we are waiting on specific information required to resolve these but the following image is a general high level system design. Variable resistors will be placed in series with one coil in each axis in order to calibrate the resistances to achieve an equal current split. Each axis will be individually controlled.
...
This project is still in progress and as a result, the design is subject to change. As such, I will not be adding any further information on the design to this page unless it is finalized. **_This will be a working page._**
